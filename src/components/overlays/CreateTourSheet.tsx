...
import { X, MapPin, Check, Clock, AlertTriangle, Navigation, Headphones, Search, Loader2 } from 'lucide-react';
    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-8 py-2.5 rounded-xl bg-canvas border border-sand-200 text-sm text-heading placeholder-gray-400 focus:outline-none focus:border-gamana-400 transition-colors"
        />
        {isSearching && (
...
export default function CreateTourSheet({
  isOpen,
  onClose,
  unlockedItems,
  unlockedLoading,
  ...
  }, [canSave, title, description, selectedStoryIds, pinnedStops, onSave, handleClose]);
  ...
  return (
    <div className="absolute inset-0 z-50 flex flex-col justify-end">
      ...
      <div
        className={`relative bg-surface rounded-t-3xl shadow-elevated transition-transform duration-200 ease-out max-h-[70vh] flex flex-col ${
      ...
      >
        ...
        <div className="px-5 pt-2 pb-2 flex items-center justify-between">
          <h3 className="text-base font-semibold text-heading">Create a Tour</h3>
          <button
            onClick={handleClose}
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-4">
          <div className="mb-4">
            <input
              ...
              className="w-full px-4 py-3 rounded-xl bg-canvas border border-sand-200 text-sm text-heading placeholder-gray-400 focus:outline-none focus:border-gamana-400 focus:ring-1 focus:ring-gamana-400 transition-colors"
            />
            <textarea
              value={description}
              ...
              rows={2}
              className="w-full px-4 py-3 mt-2 rounded-xl bg-canvas border border-sand-200 text-sm text-heading placeholder-gray-400 focus:outline-none focus:border-gamana-400 focus:ring-1 focus:ring-gamana-400 transition-colors resize-none"
            />
          </div>
          ...
          <div className="mb-4">
            ...
            {unlockedLoading ? (
              ...
              <div className="flex flex-col gap-1.5 max-h-[232px] overflow-y-auto scrollbar-hide rounded-xl">
                {activeItems.map(({ story, expires_at }) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-3 p-3 rounded-xl transition-all flex-shrink-0 ${
                        isSelected
                          ? isExpiringSoon
                            ? 'bg-amber-50 border border-amber-300'
                            : 'bg-gamana-50 border border-gamana-300'
                          : isExpiringSoon
                ...
                {expiredItems.map(({ story }) => (
                  <div
                    key={story.id}
                    className="flex items-center gap-3 p-3 rounded-xl bg-surface-alt opacity-50 flex-shrink-0"
                  >
                ...
                {(activeItems.length + expiredItems.length) > 4 && (
                  <p className="text-[10px] text-muted text-center py-1 flex-shrink-0">Scroll for more</p>
                )}
              </div>
            )}
          </div>
            ...
            {MAPS_API_KEY ? (
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2.5 rounded-xl bg-canvas border border-sand-200 text-sm text-heading placeholder-gray-400 focus:outline-none focus:border-gamana-400 transition-colors"
        ...
        <div className="px-5 pt-3 pb-8 border-t border-sand-100">
